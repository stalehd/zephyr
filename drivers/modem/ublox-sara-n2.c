...
#define LOG_DOMAIN modem_ublox_sara_n2
...
#if !defined(CONFIG_MODEM_UBLOX_SARA_N2_MANUAL_MCCMNO)
#define CONFIG_MODEM_UBLOX_SARA_N2_MANUAL_MCCMNO ""
#endif
...
#define MDM_UART_DEV_NAME DT_INST_0_UBLOX_SARA_N2_BUS_NAME
...
K_THREAD_STACK_DEFINE(modem_rx_stack, CONFIG_MODEM_UBLOX_SARA_N2_RX_STACK_SIZE);
struct k_thread modem_rx_thread;

/* RX thread work queue */
K_THREAD_STACK_DEFINE(modem_workq_stack,
		      CONFIG_MODEM_UBLOX_SARA_N2_RX_WORKQ_STACK_SIZE);
...
static int send_at_cmd(struct modem_socket *sock, const u8_t *data, int timeout)
{
	int ret;
...
static int send_data(struct modem_socket *sock, const struct sockaddr *dst_addr,
		     int dst_port, struct net_pkt *pkt)
{
	int ret, i;
	struct net_buf *frag;
				   ...
				   u16_t len)
{
	...
	sock->recv_pkt = net_pkt_rx_alloc_with_buffer(
		net_context_get_iface(sock->context), actual_length,
...
static void modem_rssi_query_work(struct k_work *work)
{
	...
	k_delayed_work_submit_to_queue(&modem_workq, &ictx.rssi_query_work,
...
restart:
	...
	ret = send_at_cmd(
		NULL,
		"AT+CGDCONT=1,\"IP\",\"" CONFIG_MODEM_UBLOX_SARA_N2_APN "\"",
		MDM_CMD_TIMEOUT);
	if (ret < 0) {
	...
	if (strlen(CONFIG_MODEM_UBLOX_SARA_N2_MANUAL_MCCMNO) > 0) {
		/* use manual MCC/MNO entry */
		ret = send_at_cmd(
			NULL,
			"AT+COPS=1,2,\"" CONFIG_MODEM_UBLOX_SARA_N2_MANUAL_MCCMNO
			"\"",
			MDM_CMD_TIMEOUT);
	} else {
		/* register operator automatically */
		ret = send_at_cmd(NULL, "AT+COPS=0,0",
		       ...
		       struct net_context **context)
{
	...
	local_port =
		ntohs(net_sin((struct sockaddr *)&(*context)->local)->sin_port);
	if (local_port > 0) {
...
static void offload_iface_init(struct net_if *iface)
{
	...
	net_if_set_link_addr(iface, modem_get_mac(dev), sizeof(ctx->mac_addr),
			     NET_LINK_ETHERNET);
	ctx->iface = iface;
}
...
NET_DEVICE_OFFLOAD_INIT(modem_sara_N2, "MODEM_SARA_N2", modem_init, &ictx, NULL,
			CONFIG_MODEM_UBLOX_SARA_N2_INIT_PRIORITY, &api_funcs,
			MDM_MAX_DATA_LENGTH);
